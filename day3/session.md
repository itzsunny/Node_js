...
When all data is received *end* event fires with a callback function with no arguments to mark data completeion.
```js
req.on('end', () => {
    // Send response here
})
...
If no data is send, *end* event fires automatically and store variable is "".
...
##### practice
send form data from postman and pasre it into the server.
